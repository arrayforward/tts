...
- **JWT RS256 鉴权**：OpenSSL 直接验签，无外部 JWT 库依赖；`GRPC_AUTH_ENABLED` 开关控制，**默认关闭**（内网模式）
...
默认 `GRPC_AUTH_ENABLED=false`，无需配置 JWT 密钥即可启动。生产环境开启鉴权：
...
| 变量 | 必填 | 默认 | 说明 |
|---|---|---|---|
| `GRPC_AUTH_ENABLED` | | `false` | `true` 开启 JWT 鉴权；默认关闭（内网模式） |
| `GRPC_JWT_PUBLIC_KEY_FILE` | 鉴权开启时 | — | RS256 公钥 PEM 路径 |
...
- 默认 `GRPC_AUTH_ENABLED=false`（内网免鉴权），公网部署务必设为 `true`
